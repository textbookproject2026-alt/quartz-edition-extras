...
export default ((userOpts?: Partial<GraphOptions>) => {
  const Graph: QuartzComponent = ({ displayClass, cfg, allFiles }: QuartzComponentProps) => {
    ...
    // Textbook: every page's topic slot, worked out here at build time from the
    // frontmatter (the content index the script fetches carries none). Shared
    // by the local and global graphs below.
    const topics = topicIndex(
      (allFiles ?? []).map((f) => ({
        slug: f.slug,
        frontmatter: f.frontmatter as Record<string, unknown> | undefined,
      })),
    );

    return (
      <div class={classNames(displayClass, "graph")} data-topics={JSON.stringify(topics)}>
        ...
        <ul class="graph-legend" aria-label="Topics" hidden></ul>
