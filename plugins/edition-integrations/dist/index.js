...
var DESIGN_FILE = fileURLToPath(new URL("../design.yaml", import.meta.url));
var PALETTE = {
  accent: "hex",
  accentHover: "colour",
  accentWash: "colour",
...
var HEADING = { size: "length", weight: "number", lineHeight: "number" };
var SCHEMA = {
...
var VALID = {
  hex: /^#[0-9a-f]{6}$/i,
  colour: /^(#[0-9a-f]{3,8}|rgba?\(\s*[\d.]+%?\s*,\s*[\d.]+%?\s*,\s*[\d.]+%?\s*(,\s*[\d.]+%?\s*)?\))$/i,
...
var check = (schema, value, path, errors) => {
  if (typeof schema === "string") {
    const kind = schema;
    const ok = kind === "boolean" ? typeof value === "boolean" : (typeof value === "string" || typeof value === "number") && VALID[kind].test(String(value).trim());
...
var parseDesign = (text, file = DESIGN_FILE) => {
  const errors = [];
  let raw;
  try {
  ...
  const design = errors.length ? void 0 : check(SCHEMA, raw, "", errors);
  if (errors.length) {
...
var loadDesign = (file = DESIGN_FILE) => {
  let text;
  try {
    text = readFileSync(file, "utf8");
  } catch {
    throw new Error(
      `edition-integrations: ${file} is missing. It ships beside dist/; reinstall the plugin (npx quartz plugin update edition-integrations).`
    );
  }
...
var hsl = (hex) => {
  ...
  if (max !== min) {
    ...
    h2 = max === r2 ? (g2 - b) / d2 + (g2 < b ? 6 : 0) : max === g2 ? (b - r2) / d2 + 2 : (r2 - g2) / d2 + 4;
    h2 /= 6;
  }
...
var colours = (p2) => {
  ...
  --dark: ${p2.heading};
  --secondary: ${p2.accent};
  --tertiary: ${p2.accentHover};
  --highlight: ${p2.accentWash};
  --textHighlight: ${p2.mark};
  --accent-h: ${accent.h};
  --accent-s: ${accent.s}%;
  --accent-l: ${accent.l}%;
  --tb-accent: ${p2.accent};
...
:root {${colours(d2.palette.light)}
  ...
  --tb-size-body: ${t2.body.size};
  --tb-lh-body: ${t2.body.lineHeight};
  --tb-size-lead: ${t2.lead.size};
  --tb-lh-lead: ${t2.lead.lineHeight};
  --tb-size-h1: ${t2.h1.size};
...
@media (max-width: ${layout.mobileWidth}) {
  :root {
    --tb-size-h1: ${t2.mobile.h1};
    --tb-size-h2: ${t2.mobile.h2};
...
var markLeadParagraph = (tree) => {
  const children = tree.children ?? [];
  const index = children.findIndex((n2) => isElement(n2) && n2.tagName === "h1");
  if (index === -1) return false;
  const next = children.slice(index + 1).find((n2) => !isBlank(n2));
  if (!isElement(next) || next.tagName !== "p") return false;
  const props = next.properties ??= {};
  const existing = props.className;
  const classes = Array.isArray(existing) ? existing.map(String) : typeof existing === "string" ? existing.split(/\s+/).filter(Boolean) : [];
